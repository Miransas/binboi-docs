...
Base URL: `https://api.binboi.com`. All endpoints accept and return JSON.
...
```http
Authorization: Bearer binboi_pat_a1b2c3d4_0123456789abcdef0123456789abcdef
```
...
```bash
curl https://api.binboi.com/v1/me \
...
```bash
curl https://api.binboi.com/v1/tokens \
  -H "Authorization: Bearer binboi_pat_..."
```
...
```bash
curl -X POST https://api.binboi.com/v1/tokens \
  -H "Authorization: Bearer binboi_pat_..." \
...
```json
{
  ...
  "token": "binboi_pat_a91f0b3e_4d5e6f708192a3b4c5d6e7f80910a2b3",
...
```bash
curl -X DELETE https://api.binboi.com/v1/tokens/tok_a91f0b3e \
  -H "Authorization: Bearer binboi_pat_..."
```
...
```bash
curl https://api.binboi.com/v1/tunnels/my-app \
  -H "Authorization: Bearer binboi_pat_..."
```
...
```bash
curl -X DELETE https://api.binboi.com/v1/tunnels/my-app \
  -H "Authorization: Bearer binboi_pat_..."
```

Response `204` (no body).

### `GET /v1/requests?subdomain=<sub>`

Recent requests captured for a tunnel, within the plan's retention window.

```bash
curl "https://api.binboi.com/v1/requests?subdomain=my-app" \
  -H "Authorization: Bearer binboi_pat_..."
```
